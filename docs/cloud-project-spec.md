...
| Onboarding default | early access free unlimited + 登入 only + empty placeholder | working hypothesis,實作中 review |
| AutoSave 觸發條件(CloudProject) | 沿用 v0.1 debounced PUT 邏輯(`HttpSyncEngine`) | 已有 If-Match 衝突偵測,反指標雙寫 |

## Onboarding / Quota / Pricing(working hypothesis)

§ 7.2 default 為 v0.2 working hypothesis。實作中可改;Q4 加碼 onboarding user testing 為 release blocker,release 前還會 review。

| 題目 | working hypothesis | 改動條件 |
|---|---|---|
| Free tier scene 上限 | 10 | F-1 quota UI 已 ready,可動態調 |
| Free tier 每 scene 大小 | 5MB(blob only,不含 asset) | 與 sync-protocol.md 1MB body limit 矛盾 — 實作以 sync-protocol.md 為準,5MB 是 future 目標 |
| Free tier asset 總量 | 500MB(沿用 asset-sync-protocol.md `free` plan) | 已 ready |
| Placeholder scene(新 user) | empty | 不做 tutorial scene |
