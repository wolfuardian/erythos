...
4. **Phase D — 真實 Linode 部署 + Postgres + GitHub OAuth + API**(含 share / fork / visibility) — 1 週
   ...
   - D3. 自寫 GitHub OAuth(Hono + `node:crypto` HMAC state)+ session cookie + Postgres `sessions` 表,`/auth/me` `/auth/github/start` `/auth/github/callback` `/auth/signout` 通(D3 Option C, 2026-05-09 落地;Better Auth dep 留 v0.1+ magic link adapter)— 1 天
