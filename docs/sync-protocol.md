...
> **Multi-user real-time co-edit**(v0.5+ L3 spec)走獨立 protocol,見 `docs/realtime-co-edit-spec.md`。本文件規範的 solo / 跨裝置同帳號路徑在 v0.5+ 仍適用,以 `project.collabMode: 'solo'` 表示;`collabMode: 'collab'` 走 CRDT awareness 路徑,**無 409 dialog**。兩條 code path 並存到 v1.0+。
