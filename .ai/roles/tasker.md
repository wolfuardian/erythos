...
### 「當前任務」內容**不得**用 `## ` 開頭的 subheader

模組 CLAUDE.md 的 sections（範圍限制 / 當前任務 / 通用 SOP / 慣例 / 待修項 / 上報區）都是 `## ` 層級。AT 寫的內容塞在「當前任務」section 內，**不能**也用 `## `（會被誤認為新 section，AD 還原 placeholder 時容易遺漏 → 殘骸進 PR diff，#381 教訓）。

✅ 允許：`### `、`#### `、粗體 `**...**`、列表
❌ 禁止：任何 `## ` 開頭行

例：
- ❌ `## Summary` / `## Test plan` / `## Technical approach`
- ✅ `### 檔案 1`、`### 不要做的事`、`**目標：**`
