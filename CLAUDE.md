...
所有 agent 在三種情境下必須跑對應指令,任何一項失敗就**停下來回報**,不准繼續往下做。

| 情境 | 指令 | 觸發者 |
|------|------|--------|
| 實作中(每段邏輯改動後) | `npm run build` | AD / AH 直做 |
| PR 開出前 | `npm run build`(含 type check) | AD |
| QC 審查時 | `npm run build` + 手動驗證關鍵路徑 | QC |
| Merge 後 | `npm run build` 在 main 上 | AH |

**規則**:

- 型別錯誤、build 失敗 → 不准開 PR,先修
- 不要編造 import 路徑或不存在的識別字。若 symbol 找不到,先 Grep 或停下來問,不要瞎猜
- 改動若影響到 runtime 行為(非純文字 / 設定),AH 在 merge 後需開 dev server 跑一次煙霧測試

---

## 禁區(Forbidden Paths)

以下路徑**禁止直接編輯**。需要動到的話必須先停下來告知指揮家。

| 路徑 | 性質 | 例外 |
|------|------|------|
| `dist/` | 建置產物 | 無 |
| `node_modules/` | 依賴 | 無 |
| `.claude/_archived/` | 已歸檔流程 | 重新啟用需指揮家批准 |
| `.claude/decisions/log.md` 既有條目 | append-only 長期決策 | 只能新增,不能改 / 刪 |
| `.git/` | 內部狀態 | 無 |

軟禁區(`pre-write-guard.cjs` 沒擋,但 AH / agent 應主動迴避):

- 跨模組大規模 rename → 需走 issue + QC,不准小變更直做
- `scripts/` 中含 `rm -rf` 或 publish 類破壞性指令 → 改前先 confirm
...
2. **事件驅動 + 事件順序**:Editor 發事件 → Bridge 更新 signal → 面板自動重渲染;順序 `nodeAdded → sceneReplaced`,不得反向
...
角色精簡歷史(AT / PM / EX / DV 砍除原因)見 `.claude/decisions/log.md`。
...
> 詳細回應節奏規範(短問句直答、結構成本意識、語感分級等)見 `~/.claude/CLAUDE.md` 使用者層級設定。
> 本檔只保留專案特有重點:

- 預設口語回應,只有指揮家明確要求方案時才用結構化格式
- 一輪一個決策點,不在回應末端逼指揮家連選多項
- 結構化(表格 / 編號)只在多方案抉擇時用
...
歸檔 skill 與半年複查時程見 `.claude/decisions/log.md`。
...
AH 每次 session 開始(含 `/clear` 後)的讀取優先序:

1. **必讀**:`.claude/session/current.md`(交接筆記)
2. **必讀**:`git status` / `gh pr list`(重建現況)
3. **按需讀**:`.claude/decisions/log.md`(僅當任務涉及流程 / 歷史決策)
4. **按需讀**:`MEMORY.md`(僅當任務涉及指揮家偏好 / 跨 session 原則)

session-startup skill 自動觸發前兩項。後兩項由 AH 視任務性質判斷,**不預設全讀**以免消耗 context。
