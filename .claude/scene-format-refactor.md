...
實際做法：保留 TransformControls，回調層透過 `sceneSync.getUUID(obj)` 將 Object3D 轉為 UUID 後執行 `SetTransformCommand`，與單向同步無衝突。詳 §3.4 D7、§4.8。
...
| D7 | TransformControls | **保留，回調層橋接 UUID + Command** | 直接 mutate 限定在 Three.js 拖曳期間，sealLast 後寫 SetTransformCommand 進歷史；無需暫移除 |
...
- 重複檔名 → auto-suffix（例如 `chair.glb` 已存在 → 寫成 `chair (1).glb`，依 `ProjectManager.importAsset` 規則）
- mesh component 的 source 引用專案目錄內的相對路徑（例如 `prefabs/chair.glb`，非 `file.name`）
...
### 4.8 與 Three.js 物件參照的功能（保留並橋接 UUID）

原計畫此節列為「暫時移除→重建」，實際 Phase 1~6 重構期間皆保留並運作；以下記錄各功能與單向同步的橋接方式：

| 功能 | 與 Object3D 的關係 | 橋接方式 |
|------|-------------------|---------|
| TransformControls (Gizmo) | 拖曳期間直接 mutate Object3D | 拖曳結束（mouseup）回調 `getUUID(obj)` → `SetTransformCommand` 寫入歷史 |
| Box Select | 框選後需從 Three.js 取 hits | `getUUID(obj)` → `selection.add(uuid)` |
| PostProcessing outline | OutlinePass 需要 Object3D[] 輸入 | UI 層讀 `selectedUUIDs()` → `sceneSync.getObject3D(uuid)` → outline |
...
實作與原規劃 API 不同。原規劃 `load(filePath)` 假設能從路徑直讀，實作改為由 caller 傳 buffer，並加入 IndexedDB 後端（GlbStore）做跨 reload 持久化。
...
class ResourceCache {
  private readonly cache: Map<string, Group>;

  // 解析 ArrayBuffer 並以 source（filePath）為快取鍵存入記憶體 + IndexedDB（GlbStore）
  async loadFromBuffer(source: string, buffer: ArrayBuffer): Promise<Group>;

  // 從 IndexedDB 讀回所有 GLB buffer 並填充記憶體快取（autosave restore 前呼叫）
  async hydrate(): Promise<void>;

  // 從快取中 clone 子樹（淺 clone，避免雙重 mesh 地雷）
...
// 測試用：注入 / 清除 module-level parser 取代 GLTFLoader
export function _mockParser(fn: ParserFn): void;
export function _clearParser(): void;
```

**關鍵差異與理由**：
- `loadFromBuffer(source, buffer)` 取代 `load(filePath)`：載入路徑由 caller（`gltfLoader.ts`）負責，ResourceCache 只負責 parse + cache
- 新增 `hydrate()` + IndexedDB 後端：解決 page reload 後快取消失問題
- `cloneSubtree` 改為淺 clone（`clone(false)`）：避免 GLTF 子樹雙重 mesh 渲染地雷
- nodePath 採 `|` 分隔（如 `"Body|Arm|Hand"`），由 `findByPath` helper 逐層導航
