...
- Wireframe：待定（未在此 spec 討論）

### 3. HDRI：scene.environment render-time swap

**規則**：HDRI asset 一份（全域載入），但每個 viewport 渲染時決定是否啟用。

**實作**：在每個 viewport 的 render override 裡：
```
scene.environment = vp.hdriEnabled ? sharedHdriTexture : null;
composer.render();
scene.environment = null;  // restore
```
...
**Shading mode 對 HDRI 啟用規則**（指揮家確認）：
- Solid / Shading：HDRI 無效，不開關
- Rendering：HDRI 有效，由環境面板開關
...
- **Wireframe mode 的燈光 / HDRI 規則**：本 spec 未討論，實作時再補
