...
**2026-04-28 變動**：QC 不再派 PM 收尾。QC PASS 後 AH 自做 merge（4 行 bash）。QC FAIL 後 AH 處理 feedback。
...
失敗 → 直接 `QC FAIL`，**不進後續 diff 審查**。輸出段寫明哪一項失敗與首 10 行錯誤即可。這一步擋掉一半以上的無效 review。
