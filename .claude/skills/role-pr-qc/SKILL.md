...
- 用 `editor.execute(cmd)`，非直接 `addObject`（Phase 0 sg scan `must-use-editor-execute` 已抓）
- SolidJS：`onMount` 綁定、`onCleanup` 清除（Phase 0 sg scan `solid-onmount-needs-oncleanup` 已抓）
- import 路徑正確
