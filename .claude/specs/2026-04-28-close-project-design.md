...
## 10. 設計決策：為何不做 Switch Project

Switch 在語意上 ≡ Close + Open。Close Project 後使用者落到 Welcome，Welcome 已提供 `Recent Projects` 列表 + `Open Folder…`，可直接挑下一個 — 走完的路徑跟「Switch Project picker」一模一樣。

額外加一個 Switch Project entry 會：
- 在已有 Welcome 等價路徑時引入第二條入口（UI 冗餘）
- 引入「不先關當前 project 就直接切」的 unsaved 風險翻倍場景（autosave 錯誤需要更複雜的 confirm 流程）
- 增加 dropdown 維護面積，但實際使用頻率與 Close 重疊

結論：**不做**。dropdown 永遠只有 `Close Project` 一項；要切專案就 close 後在 Welcome 選。
...
| 2026-04-28 | 移除 Switch Project follow-up — Close 後 Welcome 已提供等價切換路徑，多此 entry 為冗餘 UI（指揮家洞察） |
