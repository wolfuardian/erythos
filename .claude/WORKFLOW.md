...
- EX 按需探勘模組現況，寫入 `.claude/module-cache/<module>.md`
...
3. spawn MP（背景）→ 產出 `.claude/previews/<topic>.html`
4. 指揮家挑定方案
5. AH 在 issue body 加 `Mockup: .claude/previews/<topic>.html` 行
6. 進入標準路徑
...
1. AH 起 dev server + 準備截圖（親自 Playwright 或 seed script）→ 存 `.claude/audits/<panel>/`
2. spawn DV（背景）→ 讀圖 + 對照 `theme.css` → 產出 `.claude/audits/<panel>.md` 美感問題清單（中文、不含修法建議）
...
1. 查 `.claude/module-cache/<module>.md` 有無相關條目
...
- **DB-first**：要理解模組現況時先查 `.claude/module-cache/<module>.md`，不直接讀 src
- **不自己跨模組探勘**：資訊不足直接 spawn EX
- **不自己讀大檔**：超過 100 行的 src 交給 EX / AT / AA，AH 只看摘要
- **派 MP / EX / DV 前不讀整檔 src**：subagent SKILL 規定它自讀指定檔。AH 用 Glob / Grep 確認檔案存在 + 大致位置即可，整檔 Read 屬儀式（elon-review 教訓 2026-04-27）
...
1. **完整程式碼**（適用 AT → AD）：before / after 片段或整檔 content，不用 `// ...` 省略。**MP / EX / DV 例外**：只給「參考檔絕對路徑 + 章節提示」，subagent 自讀；抄 src 進 prompt 等於 byte 流動 3 次（AH 讀 → 抄 prompt → subagent 讀），且違背 SKILL 自讀設計
...
要求：先查 `.claude/module-cache/<module>.md` 現況、對照 src 驗證後更新 DB、回報精要 ≤ 150 字。
...
要求：先查 `.claude/module-cache/<module>.md`、產出塞入「當前任務」section 的內容（不含 `## ` 標題）。
...
要求：產出 `.claude/audits/<panel>.md` 中文視覺問題清單。不給修法建議、不做 code review。
