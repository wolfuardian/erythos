...
describe('serializeToPrefab', () => {
  it('produces a PrefabAsset with version 1 and matching name', () => {
    const node = makeNode();
    const asset = serializeToPrefab(asNodeUUID('uuid-root'), [node], 'Chair');
  ...
  it('does not mutate the original node', () => {
    ...
    serializeToPrefab(asNodeUUID('uuid-root'), [node], 'Chair');
  ...
  it('assigns localId 0 to root, sequential ids to children', () => {
    const root = makeNode({ id: asNodeUUID('r'), name: 'Root' });
    const child = makeNode({ id: asNodeUUID('c'), name: 'Child', parent: asNodeUUID('r') });
    const asset = serializeToPrefab(asNodeUUID('r'), [root, child], 'Group');
...
describe('deserializeFromPrefab', () => {
  ...
  it('sets root parentUUID from argument', () => {
    const node = makeNode();
    const asset = serializeToPrefab(asNodeUUID('uuid-root'), [node], 'Chair');
    const nodes = deserializeFromPrefab(asset, asNodeUUID('parent-scene-uuid'));
